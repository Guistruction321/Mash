...
function mash(){
	userHome = process.argv[2]
	if (typeof userHome == "undefined") {
		
	}
	var mash2of3 = ("You will live in a " + gethome(userHome) + ", and you will have " + getChildrenCount() + " kids!");
	return mash2of3;
}
...
function gethome(userHome){
	let homelist;
	if (typeof userHome != "undefined") {
		homelist = ["Mansion", "Apartment", "Shack", "House", userHome];
	}else{
		homelist = ["Mansion", "Apartment", "Shack", "House"];
	}

	rand = Math.random()*homelist.length;
    rand = Math.floor(rand);
    let randomHome = homelist[rand];
    return randomHome;
}
...
function getChildrenCount(){
	var childcount = Math.random()*100;

	if (typeof process.argv[3] != "undefined") {
		return Math.random() > .5 ? Math.floor(childcount) : process.argv[3];
	}else{
		return Math.floor(childcount);
	}
}
